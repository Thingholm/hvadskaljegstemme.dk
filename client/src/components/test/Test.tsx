...
export default function Test({
    ...
    const [questionIndex, setQuestionIndex] = useState(
        Object.keys(userAnswers).length < bills.length
            ? bills.findIndex((bill) => !(bill.id in userAnswers)) || 0
            : 0
    );
    ...
    const handleFinalize = (
        updatedUserAnswers: Record<number, VoteWithSkip>
    ) => {
        if (typeof window === "undefined") return;

        if (
            Object.values(updatedUserAnswers).filter(
                (vote) => vote !== "skip"
            ).length < 1
        ) {
            setValidationMessage(
                "Ingen forslag er besvaret. Venligst besvar mindst et forslag for at afslutte testen og se dit resultat."
            );
            return;
        }
            ...
            <div>
                ...
                <div className="flex flex-col gap-4 items-center bg-white p-4 md:p-8 border-t border-gray-200 md:border-none md:mx-16 lg:mx-32 md:rounded-xl md:shadow-md xl:max-w-5xl xl:mx-auto xl:pt-10">
                    <Badge className="capitalize">
                        ...
                        {currentBill.isPassed ? " - Vedtaget" : ""}
                    </Badge>
                    <h2 className="text-xl text-pretty font-bold text-center">
                        {currentBill.question}
                    </h2>
                    <Button
                        variant="mobileText"
                    ...
                    <div className="flex flex-col w-full md:w-fit md:flex-row gap-4 lg:gap-6 mt-2">
                        <TestAnswerButton
                            answer="for"
                            onClick={() => handleUserAnswer("for", currentBill)}
                            isAnswered={userAnswers[currentBill.id] === "for"}
                        />
                        <TestAnswerButton
                            answer="neither"
                            onClick={() =>
                                handleUserAnswer("neither", currentBill)
                            }
                            isAnswered={
                                userAnswers[currentBill.id] === "neither"
                            }
                        />
                        <TestAnswerButton
                            answer="against"
                            onClick={() =>
                                handleUserAnswer("against", currentBill)
                            }
                            isAnswered={
                                userAnswers[currentBill.id] === "against"
                            }
                        />
                    </div>
                    <Button
                    ...
                    <div className="hidden md:grid grid-cols-3 w-[calc(100%-2rem)] justify-between py-4 lg:pt-6 mt-8 border-t border-gray-200 ">
                        ...
                        <Button
                            ...
                            onClick={() =>
                                handleUserAnswer("skip", currentBill)
                            }
                        >
                            {questionIndex < bills.length - 1
                                ? "Spring forslaget over"
                                : "Spring forslaget over og afslut"}
                        </Button>
                        <Button
                            onClick={() => handleUserAnswer(null, currentBill)}
                    ...
                    {(validationMessage?.length ?? 0) > 0 && (
                        <p className="text-red-600 text-sm md:text-center">
                            {validationMessage}
                        </p>
                    )}
                </div>
